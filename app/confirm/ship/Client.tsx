...
import { apiRequest, confirmBuySpaceship } from "../../../src/api";
...
export default function ConfirmShipClient() {
  ...
  const [intent, setIntent] = useState<BuyShipIntent | null>(null);
  ...
  useEffect(() => {
    async function run() {
      ...
      try {
        const data = await apiRequest<BuyShipIntent>("/buy-spaceship", {
          method: "POST",
          body: JSON.stringify({ wallet: walletAddress, level }),
        });
        setIntent(data);
      } catch (e: any) {
        setError(e?.message || "Failed to prepare ship purchase");
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      await confirmBuySpaceship(intent.level, signature, intent.intentId);

      try {
        window.opener?.dispatchEvent(new Event("zeruva_ship_changed"));
      } catch {}
    ...
    <div className="min-h-dvh bg-gradient-to-b from-black via-gray-950 to-black text-white p-6">
      <div className="mx-auto w-full max-w-xl">
        <div className="rounded-2xl border border-gray-800 bg-black/60 backdrop-blur p-6">
          <div className="text-2xl font-bold">Confirm Spaceship Upgrade</div>
              ...
              <div className="rounded-xl border border-gray-800 bg-black/40 p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xl font-bold">Lv {intent.level}</div>
                  </div>
